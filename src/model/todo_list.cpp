      ...
      if (rc == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
          ...
          todoItems_.emplace_back(
              id, std::string(reinterpret_cast<const char *>(title)),
              completed);
        }
        sqlite3_finalize(stmt);
      } else {
        std::cout << "Error reading todo items: " << sqlite3_errmsg(db)
                  << std::endl;
      }
    }
  }
...
void TodoList::addTodoItem(const std::string &title) {
  size_t newId = todoItems_.size() + 1;
  todoItems_.emplace_back(static_cast<int>(newId), title, false);
  ...
  if (rc) {
    ...
    std::string insertSQL =
        "INSERT INTO todo_items (title, completed) VALUES ('" + title +
        "', 0);";

    rc = sqlite3_exec(db, insertSQL.c_str(), nullptr, nullptr, nullptr);

    if (rc != SQLITE_OK) {
      std::cout << "Error inserting todo item: " << sqlite3_errmsg(db)
                << std::endl;
    }
  }
...
void TodoList::removeTodoItem(int itemId) {
  ...
  if (rc) {
    ...
    std::string deleteSQL =
        "DELETE FROM todo_items WHERE id = " + std::to_string(itemId) + ";";

    rc = sqlite3_exec(db, deleteSQL.c_str(), nullptr, nullptr, nullptr);

    if (rc != SQLITE_OK) {
      std::cout << "Error deleting todo item: " << sqlite3_errmsg(db)
                << std::endl;
    }
  }
...
void TodoList::markTodoItemCompleted(int itemId) {
  ...
  if (rc) {
    ...
    if (rc != SQLITE_OK) {
      std::cout << "Error updating todo item: " << sqlite3_errmsg(db)
                << std::endl;
    }
  }
